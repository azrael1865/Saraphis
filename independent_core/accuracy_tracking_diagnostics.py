...
# Import dependency checker for graceful fallbacks
try:
    from dependency_checker import jwt, toml, yaml, print_dependency_report, get_dependency_status
except ImportError:
...
try:
    # Try absolute imports first (for direct module import)
    ...
    # Fallback to relative imports (for package-based imports)
    try:
        from .enhanced_fraud_core_monitoring import (
            MonitoringManager, MetricsCollector, PerformanceMetrics,
            ResourceMetrics, CacheManager, MonitoringConfig
        ...
        # Log the import error but continue - some classes may still be usable
        import warnings
        warnings.warn(
            f"Could not import all dependencies: {e}. "
            "Some functionality may be limited. "
            "Ensure all required modules are in the Python path.",
            ImportWarning
        )
        # Define minimal stubs to prevent complete failure
        class MonitoringManager: pass
        class MetricsCollector: pass
        class PerformanceMetrics: pass
        class ResourceMetrics: pass
        ...
        class MonitoringError(EnhancedFraudException): pass
        class ErrorContext: pass
        def create_error_context(*args, **kwargs): return {}
        class AccuracyTrackingDatabase: pass
        class DatabaseConfig: pass
        class AccuracyMetric: pass
        class ConfigManager: pass
        class ConfigManagerSettings: pass
