...
class HybridPadicConverter:
    ...
    def _extract_exponent_channel(self, coeff_tensor: torch.Tensor, prime: int) -> torch.Tensor:
        ...
        # Use log base prime to get hierarchical structure
        # This preserves ultrametric property
        
        # Add small epsilon to avoid log(0)
        epsilon = 1e-8
        log_coeffs = torch.log(coeff_tensor.float() + epsilon)
        prime_log = torch.log(torch.tensor(prime, dtype=torch.float32, device=coeff_tensor.device))
        
        exponent_channel = log_coeffs / prime_log
        
        # Clamp to reasonable range to prevent overflow
        exponent_channel = torch.clamp(exponent_channel, min=-10.0, max=10.0)
    ...
    def _extract_mantissa_channel(self, coeff_tensor: torch.Tensor, prime: int) -> torch.Tensor:
        ...
        # Normalize coefficients to [0, prime) range
        mantissa_channel = coeff_tensor % prime
        
        # Ensure values are exactly in [0, prime) range
    ...
    def _reconstruct_coefficients(self, exponent_channel: torch.Tensor, 
        ...
        """Reconstruct coefficients from two channels"""
        # Reconstruct using inverse operations
        prime_tensor = torch.tensor(prime, dtype=torch.float32, device=exponent_channel.device)
        
        # Reconstruct from exponent channel (inverse of log operation)
        exp_reconstructed = torch.exp(exponent_channel * torch.log(prime_tensor))
        
        # Combine with mantissa channel
        # Use weighted combination to preserve both channels
        alpha = 0.7  # Weight for exponent channel
        beta = 0.3   # Weight for mantissa channel
        
        reconstructed = alpha * exp_reconstructed + beta * mantissa_channel
